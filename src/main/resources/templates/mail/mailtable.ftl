            ...
            <#if mapList??>
                <#list mapList as list>
                    <tr>
                        ...
                        <td><span>${list.mailNumberPOName}</span></td>
                        <td><span>${list.mailCreateTime}</span></td>
                        <td><span class="label ${list.statusColor}">${list.statusName}</span>
                        </td>
                        <td><a href="addaccount?id=${list.mailNumberPOId}" class="label xiugai">
                                <span class="glyphicon glyphicon-edit"></span> 修改</a>
                            <a onclick="{return confirm('删除该记录将不能恢复，确定删除吗？');};"
                               href="delete?id=${list.mailNumberPOId}" class="label shanchu">
