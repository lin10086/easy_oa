...
<div class="row" style="padding-top: 15px;">
            ...
            <div class="box-body">
                <div class="input-group">
                    ...
                    <div class="input-group-btn">
                        <input type="submit" class="btn btn-primary addtype" name="" id="" style="padding:6px;"
                               value="新增"/>
                    </div>
                </div>
            </div>
    ...
    <div class="col-md-9">
        ...
        <div class="bgc-w box box-primary thistableson">
...
<script type="text/javascript">
    $(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data) {
                        ...
                        $('.thistableson').load('outaddresspaging', {alph: alph});
                        modalShow(1);
                    }
                }
            })
        ...
        /* 点击移动的事件 */
        $('.thistableson').on('click', '.thismove', function () {
            //设置id
        ...
        /* 外部通讯录分会的点击事件 */
        $('.thistableson').on('click', '.returnoutaddress', function () {
            var alph = $('#thispills .active a').text().trim();	//获取字母表中的值
            var outtype = $('#thisul .activee').text().trim();	//获取外部通讯录的分类名称
            $('.thistableson').load('outaddresspaging', {alph: alph, outtype: outtype});
        })
        /*内部通讯录返回点击事件  */
        $('.thistableson').on('click', '.returninaddress', function () {
            var alph = $('#thispills .active a').text().trim();	//获取字母表中的值
            $('.thistableson').load('inaddresspaging', {alph: alph});
        })
        /* 查看外部联系人详情 */
        $('.thistableson').on('click', '.outlookthis', function () {
            var director = $(this).attr('director');
            $('.thistableson').load('outmessshow', {director: director});
        })
        /* 查看内部联系人详情 */
        $('.thistableson').on('click', '.inlookthis', function () {
            var userId = $(this).attr('userId');
            console.log(userId);
            $('.thistableson').load('inmessshow', {userId: userId});
        })
        /* 我的分享的消息记录 */
        $('.meshareother').on('click', function () {
            console.log("我的分享点击了么？");
            $('.thistableson').load('mesharemess');
        })
        /* 分享信息按钮的点击事件 */
        $('.sharewithme').on('click', function () {
            $('.thistableson').load('sharemess');
        })
        /*模态框关闭的按钮和确定按钮；都要将模态框里面的内容替换成分享时的内容*/
        $('.thistableson').on('click', '#shareModal .closemodal', function () {
            $("#shareModal .modal-1").load("modalshare");
        })

        /* 模态框中的点击 */
        $('.thistableson').on('click', '.thisshareuser', function () {
            console.log("分享点击了么？");
        ...
        $('.thistableson').on('click', '.thisclose', function () {
            $(this).parent().css('display', 'none');
        });
        /*分享。。。*/
        $('.thistableson').on('click', '.thisshare', function () {
        ...
        /* 用户删除某个联系人 */
        $('.thistableson').on('click', '.thisdelete', function () {
            var did = $(this).attr('did');
            console.log(did);
            if (confirm("确定删除吗？")) {
                $('.thistableson').load('deletedirector', {did: did}, function () {
                    modalShow(1);
                });
            }
        })
        ...
        $('.addaddress').on('click', function () {
            $('.thistableson').load('addaddress');
        })
        /* 修改外部联系人 */
        $('.thistableson').on('click', '.thischange', function () {
            var did = $(this).attr('did');
            console.log(did);
            $('.thistableson').load('addaddress', {did: did});
        })
        /* 刷新按钮 */
        $('.thistableson').on('click', '.thisrefresh', function () {
            var alph = $('#thispills .active a').text().trim();	//获取字母表中的值
        ...
        /* 查找关键字事件， */
        $('.thistableson').on('click', '.baseKeySumbit', function () {
            var alph = $('#thispills .active a').text().trim();	//获取字母表中的值
            var baseKey = $('.baseKey').val().trim();				//获取搜索关键字
            var type = $('#navpills .activee').text().trim();		//获取内部或外部的类型值
            var outtype = $('#thisul .activee').text().trim();	//获取外部通讯录的分类名称
            if (type == "内部通讯录") {
                $('.thistableson').load('inaddresspaging', {alph: alph, baseKey: baseKey});
            } else {
                /* 就是进入外部通讯录了， */

                $('.thistableson').load('outaddresspaging', {alph: alph, outtype: outtype, baseKey: baseKey});
            }
        })
        ...
        $('#thisul').on('click', '.thisshanchu', function () {
            if (confirm("确定删除该分类吗？该分类将被移到外部通讯录哟")) {
                ...
                $('.thistableson').load('deletetypename', {typename: typename});
            }
        ...
        $('#commentsave').on('click', function () {
            ...
            $('.thistableson').load('changetypename', {typename: typename, oldtypename: oldtypename});
