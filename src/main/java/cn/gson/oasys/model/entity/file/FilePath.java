...
public class FilePath {
    ...
    private Long parentId;//文件路径的上级路径id

    @Column(name = "path_name")
    private String pathName;//文件路径名

    @Column(name = "path_istrash")
    private Long pathIstrash = 0L;//是否是垃圾文件

    @Column(name = "path_user_id")
    private Long pathUserId; //用户id

    @OneToMany(mappedBy = "fpath")
    @JsonIgnore
    private List<FileList> fileList;//文件列表
