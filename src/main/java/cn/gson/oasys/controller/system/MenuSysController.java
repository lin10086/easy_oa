...
*/
  ...
  /*  @Resource
    ...
    *//**
    ...
    @RequestMapping("testsysmenu")
    public String testsysmenu(HttpServletRequest req) {
        List<SysMenuPO> oneSysMenuPOList = systemMenuServiceV2.oneSysMenuPOListAll(0L);//父菜单
        List<SystemMenuVO> oneSysMenuVOListAll = SystemMenuListFactoryVO.createSystemMenuVOList(oneSysMenuPOList);
        List<SysMenuPO> twoSysMenuPOList = systemMenuServiceV2.twoSysMenuPOListAll(0L);//子菜单
        List<SystemMenuVO> twoSysMenuVOListAll = SystemMenuListFactoryVO.createSystemMenuVOList(twoSysMenuPOList);
    ...
    *//**
    ...
    public String newpage(HttpServletRequest req) {
        ...
        List<SysMenuPO> oneSysMenuPOList = systemMenuServiceV2.oneSysMenuPOListAll(0L);//父级菜单
//        List<SystemMenu> parentList = iDao.findByParentIdOrderBySortId(0L);
        req.setAttribute("parentList", oneSysMenuPOList);
        HttpSession session = req.getSession();
        session.removeAttribute("getId");//菜单ID
        if (!StringUtils.isEmpty(req.getParameter("id"))) {
            Long getId = Long.parseLong(req.getParameter("id"));
            SysMenuPO sysMenuPO = systemMenuServiceV2.getSysMenuPOByMenuId(getId);
            SystemMenu menuObj = iDao.findOne(getId);
            if (!StringUtils.isEmpty(req.getParameter("add"))) {
                Long getAdd = menuObj.getMenuId();
                String getName = menuObj.getMenuName();
